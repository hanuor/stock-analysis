...
export const priceSort = () => {
	const sortFunc = (a: Row, b: Row, columnId: string, desc: boolean) => {
		...
		if (splitA.length > 1) {
			aNumber =
				(Number(splitA[0].substring(1)) + Number(splitA[4].substring(1))) /
				2;
		} else {
		...
		if (splitB.length > 1) {
			bNumber =
				(Number(splitB[0].substring(1)) + Number(splitB[4].substring(1))) /
				2;
		} else {
